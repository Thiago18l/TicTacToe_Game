...
class Game < Gosu::Window
  ...
  def draw
    ...
    # display message
    display_message("Você ganhou!") if @position.win?("x")
    display_message("Computador Ganhou!") if @position.win?("o")
    display_message("EMPATE!") if @position.possible_moves.empty?
  end
  def display_message(txt)
    ...
    draw_quad(0,100,black,
              ...
              width, 500, black,
              0, 500, black)
    @mfont.draw_text(txt, (width-@mfont.text_width(txt))/2, width/2 - 100, 0)
  end
end
